...
function checkUserRegistration(email) {
    const databaseEndpoint = 'https://database.binaryfx.site/api/1.1/obj/Deriv Hacker';
...
function sendToBubbleApp(data) {
    const bubbleEndpoint = 'https://database.binaryfx.site/api/1.1/wf/Deriv Hacker';
    ...
    if (confirm(`Are you sure you want to stop copying ${trader.name}?`)) {
        ...
        fetch(databaseEndpoint)
            ...
            .then(data => {
                ...
                setTimeout(() => {
                    const deleteTokenEndpoint = 'https://database.binaryfx.site/api/1.1/wf/delete token';
